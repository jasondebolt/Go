...
export function fetchContext() {
  const request = axios.get(`${FULL_API_URL}/context`)
  return {
    type: FETCH_CONTEXT,
    payload: request
  };
...
export function fetchLinks() {
  const request = axios.get(`${FULL_API_URL}/links`)
  return {
    type: FETCH_LINKS,
    payload: request
  };
...
export function putLink(values, successCallback, errorCallback) {
  ...
  const request = axios.put(`${FULL_API_URL}/links`, values)
...
export function fetchLink(alias) {
  const request = axios.get(`${FULL_API_URL}/links/${alias}`)
...
export function deleteLink(alias, successCallback, errorCallback) {
  console.log(`${FULL_API_URL}/links/${alias}`)
  const request = axios.delete(`${FULL_API_URL}/links/${alias}`)
    .then((response) => successCallback(response))
    .catch((response) => errorCallback(response))
